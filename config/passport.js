...
const getRoleFromState = (state) => {
  ...
  try {
    ...
    return ['client', 'freelancer'].includes(parsed.role) ? parsed.role : 'client';
  } catch (error) {
    try {
...
const getNamesFromProfile = (profile) => {
  ...
  const lastName = profile.name?.familyName || parts.slice(1).join(' ') || 'User';
...
const findOrCreateOAuthUser = async (req, provider, profile) => {
  ...
  if (!user) {
    ...
    user = await User.create({
      ...
      emailVerified: true,
      isVerified: true,
      oauthProviders: [{ provider, providerUserId, email }]
    });
  ...
  if (!isLinked) {
    user.oauthProviders.push({ provider, providerUserId, email });
  }
...
export const configurePassport = () => {
  ...
  const getCallbackUrl = (provider) => {
    ...
    return process.env[envKey] || `${baseUrl}/api/auth/oauth/${provider}/callback`;
  };
    ...
    passport.use(
      new GoogleStrategy(
        ...
        async (req, accessToken, refreshToken, profile, done) => {
          try {
            const user = await findOrCreateOAuthUser(req, 'google', profile);
            done(null, user);
          } catch (error) {
            done(error, null);
          }
  ...
  if (process.env.OAUTH_GITHUB_CLIENT_ID && process.env.OAUTH_GITHUB_CLIENT_SECRET) {
    passport.use(
      new GitHubStrategy(
        ...
        async (req, accessToken, refreshToken, profile, done) => {
          try {
            const user = await findOrCreateOAuthUser(req, 'github', profile);
            done(null, user);
          } catch (error) {
            done(error, null);
          }
  ...
  if (process.env.OAUTH_LINKEDIN_CLIENT_ID && process.env.OAUTH_LINKEDIN_CLIENT_SECRET) {
    passport.use(
      new LinkedInStrategy(
        ...
        async (req, accessToken, refreshToken, profile, done) => {
          try {
            const user = await findOrCreateOAuthUser(req, 'linkedin', profile);
            done(null, user);
          } catch (error) {
            done(error, null);
          }
